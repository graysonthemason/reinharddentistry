...
class GPR_License {
	...
	function edd_wordimpress_license_page() {
		...
		<div class="edd-wordimpress-license-wrap">
			<h2><?php _e( 'Plugin License', 'gpr'); ?></h2>
			...
			if ( $status !== false && $status == 'valid' ) {
				...
				<div class="license-stats list-group">
					<p class="list-group-item"><strong><?php _e( 'License Status:', 'gpr'); ?></strong>
						...
						<strong>(<?php echo $this->time_left_on_license( $license['license_expiration'] );
							_e( ' Days Remaining', 'gpr' ); ?>)</strong></p>

					<p class="list-group-item">
						<strong><?php _e( 'License Expiration:', 'gpr' ); ?></strong> <?php echo $license['license_expiration']; ?>
					</p>

					<p class="list-group-item">
						<strong><?php _e( 'License Owner:', 'gpr' ); ?></strong> <?php echo $license['license_name']; ?></p>

					<p class="list-group-item">
						<strong><?php _e( 'License Email:', 'gpr' ); ?></strong> <?php echo $license['license_email']; ?></p>

					<p class="list-group-item">
						<strong><?php _e( 'License Payment ID:', 'gpr' ); ?></strong> <?php echo $license['license_payment_id']; ?>
					</p>
				</div>

				<p class="alert alert-success license-status"><?php _e( 'Your license is active and you are receiving updates.', 'gpr' ); ?></p>
			...
			elseif ( $status == 'invalid' && isset( $license['license_error'] ) && $license['license_error'] == 'no_activations_left' ) {
				...
				<p class="alert alert-red license-status"><?php _e( 'The license you entered has reached the activation limit. To purchase more licenses please visit WordImpress.', 'gpr' ); ?></p>

			<?php } elseif ( $status == 'invalid' && isset( $license['license_error'] ) && $license["license_error"] == 'missing' ) { ?>

				<p class="alert alert-red license-status"><?php _e( 'There was a problem with the license you entered. Please check that your license key is active and valid then reenter it below. If you are having trouble please contact support for assistance.', 'gpr' ); ?></p>

			<?php } else { ?>

				<p class="alert alert-red license-status"><?php _e( 'Activate your license to receive automatic plugin updates for the life of your license.', 'gpr' ); ?></p>
			...
			<form method="post" action="options.php">
				...
				<label class="description licence-label" for="<?php echo $this->licence_key_option; ?>"><?php if ( $status !== false && $status == 'valid' ) {
						_e( 'Your licence is active and valid.', 'gpr' );
					} else {
						_e( 'Enter your license key to receive updates and support', 'gpr' );
					} ?></label>


				<?php if ( $status !== false && $status == 'valid' ) { ?>
					<?php wp_nonce_field( 'edd_wordimpress_nonce', 'edd_wordimpress_nonce' ); ?>
					<input type="submit" class="button-secondary deactivate-license-btn" name="edd_license_deactivate" value="<?php _e( 'Deactivate License', 'gpr' ); ?>" />
				<?php
				} else {
					wp_nonce_field( 'edd_wordimpress_nonce', 'edd_wordimpress_nonce' ); ?>
					<input type="submit" class="button-secondary activate-license-btn" name="edd_license_activate" value="<?php _e( 'Activate License', 'gpr' ); ?>" />
				<?php } ?>
