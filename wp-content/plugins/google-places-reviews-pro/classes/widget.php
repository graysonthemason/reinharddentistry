...
class Google_Places_Reviews extends WP_Widget {
	...
	public function __construct() {
		...
		$this->api_key = isset( $this->options['google_places_api_key'] ) ? $this->options['google_places_api_key'] : '';
	...
	function add_gpr_admin_widget_scripts( $hook ) {
		...
		if ( $hook == 'widgets.php' ) {
			...
			wp_enqueue_media();
			wp_enqueue_script( 'gpr_widget_admin_scripts', plugins_url( 'assets/js/admin-widget' . $suffix . '.js', dirname( __FILE__ ) ), array(
				'jquery',
				'media-upload',
				'media-views'
			) );
	...
	function grp_plugin_curl( $url ) {
		...
		if ( isset( $response['result']['reviews'] ) && ! empty( $response['result']['reviews'] ) ) {
			...
			foreach ( $response['result']['reviews'] as $review ) {
				...
				$avatar_img      = preg_replace( "/^http:/i", "https:", $avatar_get_body['entry']['gphoto$thumbnail']['$t'] ); //ensure always loading HTTPS images
	 ...
	 * @param bool  $id_prefix
	 *
	 * @return string image html
	 */
	private function get_image_html( $instance, $id_prefix ) {
		...
		$instance['width']  = isset( $instance['width'] ) ? abs( $instance['width'] ) : 60;
		$instance['height'] = isset( $instance['height'] ) ? abs( $instance['height'] ) : 60;
		...
		$output .= '<a href="#" class="gpr_remove_image" onclick="gpr_avatar_widget.removeImage(\'' . $id_prefix . '\'); return false;">' . __( 'Remove Avatar', 'gpr' ) . '</a>';
